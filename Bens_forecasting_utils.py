...
SECRET_PATH = '/Users/krisi.afezolli/Desktop/Folder/credentials.json' # path to the credentials.json file for Google API authentication
...
def main(input_sheet_id, input_sheet_range, holidays_sheet_id='1DDMBg8-q5SuPpeL1ZqC8vs7D9U-_b_F0dIroFBsNvEk', holidays_sheet_range='holidays!A1:E', stddev_threshold=1.8, outlier_rolling_window=13, monthly_seasonality_window = 14, wrap_monthly_seasonality=True, weekly_seasonality_window = 16, enable_verbose_logging=False):
    ...
    # Ensure the 'exclude_from_seasonality_calcs_flg' column exists and initialize it as False
    if 'exclude_from_seasonality_calcs_flg' not in df_raw_vol.columns:
        df_raw_vol['exclude_from_seasonality_calcs_flg'] = False

    # Set it to "True" based on conditions (outliers and holidays)
    ...
    # Proceed with other operations as needed
...
def get_intra_month_seasonality(df_raw_vol, value_col='temp_value', window_size=14, wrap_spline=True):
    ...
    # Fill NaN values in the 'seasonality' column with the mean value
    df_raw_vol['seasonality'] = df_raw_vol['seasonality'].fillna(value=ss_mean) # improved this line
...
def get_intra_week_distro(df_raw_vol, window_size = 16):
    ...
    # Fix dtype mismatch
    df_raw_vol.loc[df_raw_vol['exclude_from_seasonality_calcs_flg'] == True, 'temp_value'] = df_raw_vol['rolling_mean'].astype('float32')
    df_raw_vol['temp_value'] = pd.to_numeric(df_raw_vol['temp_value'], downcast="float")

    # Filter recent window of dates
    df_raw_vol['date'] = pd.to_datetime(df_raw_vol['date'])
    max_date = max(df_raw_vol['date'])
    min_date = max_date - timedelta(days = window_size * 7)
    df_raw_vol = df_raw_vol[df_raw_vol['date'] >= min_date]

    # Compute intra-week metrics
    df_raw_vol = df_raw_vol.groupby(['dow', 'business_line_alias']).mean(numeric_only=True)
    df_raw_vol['seasonality'] = df_raw_vol.groupby('business_line_alias')['temp_value'].transform(lambda x: (x - x.mean()) / x.mean())
    df_raw_vol['intra_week_distro'] = df_raw_vol['temp_value'] / df_raw_vol.groupby('business_line_alias')['temp_value'].transform('sum')

    df_raw_vol = df_raw_vol.reset_index()
    df_raw_vol['avg_vol'] = df_raw_vol['temp_value']

    # Drop intermediate columns
    df_raw_vol = df_raw_vol.drop(['value', 'dom', 'is_stdv_stats_outlier', 'temp_value', 'rolling_stddev',
                                   'rolling_mean', 'low_outlier_threshold', 'high_outlier_threshold',
                                   'is_holiday', 'exclude_from_seasonality_calcs_flg'], axis=1)
